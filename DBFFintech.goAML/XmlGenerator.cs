...
namespace DBFFintech.goAML
{
    public static class XmlGenerator
    {
        ...
        /// <summary>
        /// Used to generate formatted XML for goAML submission.
        /// </summary>
        /// <param name="settings">Settings used to generate file.</param>
        /// <param name="dt">Transaction data.</param>
        /// <returns>XML string ready for submission to the goAML online portal.</returns>
        public static string GenerateXML(XmlGeneratorModel settings, DataTable dt)
        {
            ...
            try
            {
                ...
                reportXML.Append("       <report_code>CTR</report_code>"); // Type of transaction (STR/PEP STR/CTR)
                ...
                //if (settings.DynamicReportingPerson)
                //{

                //    DataTable dt_ReportHeader_Dynamic = dataLayer.GET_ReportHeader_Dynamic();

                //    if (settings.LoggingAll)
                //    {
                //        if (dt_ReportHeader_Dynamic != null)
                //        {
                //            if (dt_ReportHeader_Dynamic.Rows.Count > 0)
                //            {
                //                dataLayer.LogError("Inside dt_ReportHeader_Dynamic function", "Contact found");
                //            }
                //            else
                //            {
                //                dataLayer.LogError("Inside dt_ReportHeader_Dynamic function", "Contact not found");
                //            }
                //        }
                //    }

                //    reportXML.AppendFormat("               <gender>{0}</gender>", dt_ReportHeader_Dynamic.Rows[i]["gender"].ToString());
                //    reportXML.AppendFormat("               <title>{0}</title>", dt_ReportHeader_Dynamic.Rows[i]["title"].ToString());
                //    reportXML.AppendFormat("               <first_name>{0}</first_name>", dt_ReportHeader_Dynamic.Rows[i]["first_name"].ToString());
                //    reportXML.AppendFormat("               <last_name>{0}</last_name>", dt_ReportHeader_Dynamic.Rows[i]["last_name"].ToString());
                //    //reportXML.AppendFormat("               <birthdate>1983-02-23T00:00:00</birthdate>");

                //    DateTime varbirthdate = Convert.ToDateTime(dt_ReportHeader_Dynamic.Rows[i]["birthdate"].ToString());
                //    string varbirthdateFormated = varbirthdate.Date.ToString("dd-MM-yyy HH:mm:ss");

                //    reportXML.AppendFormat("               <birthdate>" + SqlHelper.varFormatDate_To_YYYY_MM_DDTHH_MM_SS(varbirthdateFormated) + "</birthdate>");
                //    //25 June 2019 Militant
                //    //reportXML.AppendFormat("               <id_number>{0}</id_number>", dt_ReportHeader_Dynamic.Rows[i]["id_number"].ToString());
                //    reportXML.AppendFormat("               <nationality1>BW</nationality1>");
                //    reportXML.AppendFormat("          <phones>");
                //    reportXML.AppendFormat("            <phone>");

                //    reportXML.AppendFormat("               <tph_contact_type>2</tph_contact_type>");
                //    reportXML.AppendFormat("               <tph_communication_type>M</tph_communication_type>");
                //    reportXML.AppendFormat("               <tph_country_prefix>BW</tph_country_prefix>");
                //    reportXML.AppendFormat("               <tph_number>{0}</tph_number>", dt_ReportHeader_Dynamic.Rows[i]["tph_number"].ToString());
                //    //reportXML.AppendFormat("               <tph_extension>{0}</tph_extension>", dt_ReportHeader_Dynamic.Rows[i]["tph_extension"].ToString());
                //    reportXML.AppendFormat("               <tph_extension>2222</tph_extension>");
                //    reportXML.AppendFormat("               <comments>{0}</comments>", dt_ReportHeader_Dynamic.Rows[i]["comments"].ToString());

                //    reportXML.AppendFormat("          </phone>");
                //    reportXML.AppendFormat("         </phones>");

                //    reportXML.AppendFormat("            <email>{0}</email>", dt_ReportHeader_Dynamic.Rows[i]["email"].ToString());
                //}
                //else
                //{
                ...
                reportXML.Append("        <location>" +
                               ...
                               "        </location>");
                reportXML.Append("       <reason>n/a</reason>"); //Optional Can be null - Hard Coded
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    try
                    {
                        ...
                        transactionXML.AppendLine("       <birth_place>"+dt.Rows[i]["p_country"].ToString()+"</birth_place>");
                        //11 June 2019 Militant - Check if OMANG and add <id_number> else Don`t 

                        if (identification_type == "B")
                        {
                        ...
                        {
                            //transactionXML.AppendLine("       <passport_country>" + dt.Rows[i]["p_country"].ToString() + "</passport_country>");
                            transactionXML.AppendLine("       <passport_number>" + dt.Rows[i]["P_NATIONAL_ID"].ToString() + "</passport_number>");
                        }
                        transactionXML.AppendLine("       <nationality1>"+dt.Rows[i]["country_id"].ToString()+"</nationality1>");
                        ...
                        */
                        ...
                        else if (identification_type == "C") // Other
                        {
                            transactionXML.AppendLine("              <type>C</type>");
                        }
                        ...
                        transactionXML.AppendLine("              <issue_country>" + dt.Rows[i]["country_id"].ToString() + "</issue_country>");
                        transactionXML.AppendLine("            </identification>");
                        ...
                        transactionXML.AppendLine("       <from_country>" + dt.Rows[i]["country_id"].ToString() + "</from_country>"); // Optional
